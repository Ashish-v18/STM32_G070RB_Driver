 ...
 * @return           - None
 *
 * @note             - This function is essential for enabling the clock before any GPIO configuration or operation.
 *                     Without enabling the clock, GPIO functions may not work as expected.
 *******************************************************/
 ...
 * @fn              - GPIO_Init
 *
 * @brief           - Initializes the specified GPIO pin by configuring the mode, speed,
 *                    pull-up/pull-down settings, output type, and alternate functionality
 *                    (if applicable).
 *
 * @param[in]       - pGPIOHandle : Pointer to the GPIO handle structure that contains
 *                    the configuration information for the specified pin.
 *
 * @return          - None
 *
 * @note            - This function configures a GPIO pin’s mode (input, output, alternate
 *                    function, or analog), speed, pull-up/pull-down resistor settings,
 *                    output type, and alternate functionality (if the pin is set to
 *                    alternate mode). It does not handle interrupt configuration,
 ...
 * @return           - None
 *
 * @note             - This function resets all registers of the specified GPIO port,
 *                     essentially "de-initializing" the port and bringing it back to its default state.
 *                     Use with caution as it will clear all configurations for that GPIO port.
 *******************************************************/
...
void GPIO_WriteToOutputPin(GPIO_RegDef_t *pGPIOx, uint8_t PinNumber, uint8_t Value)
{
    ...
    {
        pGPIOx->BSRR = (1 << PinNumber); // Set bit atomically
    }
    else
    {
        pGPIOx->BSRR = (1 << (PinNumber + 16)); // Reset bit atomically
    }
}
